  ...
  "A Processor" when {
    ...
    "Loaded with a Job after receiving materials" should {
      ...
      "be in use" in {
        ...
        underTest.wipFor("anything") shouldBe empty
        underTest.inWip("anything") shouldBe false
        underTest.wipFor(jobSpec1.id) should contain (underTest.WIP(jobSpec1, mats1, JobProcessingState.LOADED, 5))
    ...
    "Loaded with three jobs" should {
      ...
      "be busy" in {
        ...
        underTest.wipFor("anything") shouldBe empty
        underTest.inWip("anything") shouldBe false
        underTest.wipFor(jobSpec2.id) should contain (underTest.WIP(jobSpec2, mats2, JobProcessingState.LOADED, 10))
    ...
    "Loaded with one job" should {
      ...
      "Move a Job through its lifecycle" in {
        ...
        underTest.wipFor(jobSpec1.id) should contain (underTest.WIP(jobSpec1, mats1, JobProcessingState.IN_PROGRESS, 5, 20))
        underTest.completeJob(25, jobSpec1.id).value shouldBe expectedResult
        underTest.wipFor(jobSpec1.id) should contain (underTest.WIP(jobSpec1, mats1, JobProcessingState.COMPLETE, 5, 20, 25, result=expectedResult._1, product=expectedResult._2))
