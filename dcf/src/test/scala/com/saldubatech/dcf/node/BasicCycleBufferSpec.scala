...
class BasicCycleBufferSpec extends BaseSpec {
    ...
    "it has accepted a material item" should {
      ...
      "be able to pack it" in {
        (for {
          ...
          packed.material.components shouldBe available.map(_.material)
        }).value
      }
    }
    "it has accepted a material item and packed it" should {
      ...
      "show it as ready" in {
        (for {
          ...
          ready.head.material.components should equal (List(probe))
        }).value
      }
    }
  }
