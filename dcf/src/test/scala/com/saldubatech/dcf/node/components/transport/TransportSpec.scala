...
class TransportSpec extends BaseSpec:
  ...
  def buildUnderTest(
    ...
    def dPhysics(host: Discharge.API.Physics): Discharge.Environment.Physics[ProbeInboundMaterial] =
      Harness.MockDischargePhysics[ProbeInboundMaterial](() => dischargeDelay, engine)
    def tPhysics(host: Link.API.Physics): Link.Environment.Physics[ProbeInboundMaterial] =
      Harness.MockLinkPhysics[ProbeInboundMaterial](() => transportDelay, engine)
    def iPhysics(host: Induct.API.Physics): Induct.Environment.Physics[ProbeInboundMaterial] =
      Harness.MockInductPhysics[ProbeInboundMaterial](() => inductDelay, engine)
    ...
    val linkAcknowledgeFactory: Link[ProbeInboundMaterial] => Link.API.Downstream = l =>
    ...
    val cardRestoreFactory
        : Discharge[ProbeInboundMaterial, Discharge.Environment.Listener] => Discharge.Identity & Discharge.API.Downstream = d =>
      Harness.MockAckStub(d.id, d.stationId, d, engine)
    TransportImpl[ProbeInboundMaterial, Induct.Environment.Listener, Discharge.Environment.Listener](
      "underTest", iPhysics, None, inductStore, tPhysics, dPhysics, inductUpstreamInjector, linkAcknowledgeFactory,
      cardRestoreFactory
    )
  ...
  "A Transport" when {
    ...
    "It is provided with cards" should {
      ...
      "initiate a discharge when requested" in {
        ...
        engine.pending.size should be(1)
        engine.pending(dischargeDelay).size should be(1)
      }
