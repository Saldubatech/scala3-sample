...
//  import Harness._
  ...
  def buildSourceMachineUnderTest[M <: Material: Typeable](
      engine: MockAsyncCallback,
      loads: Seq[M]
  ): AppResult[(TransportHarness.MockSink[M], SourceMachine[M], Induct[M, Induct.Environment.Listener])] =

    def obDistPhysics(host: Discharge.API.Physics): Discharge.Environment.Physics[M] =
      TransportHarness.MockDischargePhysics[M](() => obDiscDelay, engine)
    def obTranPhysics(host: Link.API.Physics): Link.Environment.Physics[M] =
      TransportHarness.MockLinkPhysics[M](() => obTranDelay, engine)
    def obIndcPhysics(host: Induct.API.Physics): Induct.Environment.Physics[M] =
      TransportHarness.MockInductPhysics[M](() => obIndcDelay, engine)
    ...
    yield
      ...
      val arrivalGenerator: (currentTime: Tick) => Option[(Tick, M)] = (currentTime: Tick) =>
        loadIt.nextOption().map((l, t) => t -> l)
  ...
  "A Load Source" when {
    ...
    "Given the command to run" should {
      "Generate as many as cards available in one run" in {
        ...
        engine.pending.size should be(loads.size + 1) // arrivals plus complete
      }
      "Send all to the provided induct once all the physics run" in {
        // Discharge Finalize
        for idx <- 0 to loads.size - 1
        yield
