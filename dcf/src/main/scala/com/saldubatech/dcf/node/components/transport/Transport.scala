...
trait Transport[
    M <: Material,
    I_LISTENER <: Induct.Environment.Listener: Typeable,
    D_LISTENER <: Discharge.Environment.Listener: Typeable
] extends Identified:
...
class TransportImpl[
  ...
  override def induct(stationId: Id, host: Induct.API.Physics): AppResult[Induct[M, I_LISTENER]] =
    _induct.fold(
      err =>
        inductPhysics = iPhysics(host)
        AppSuccess(
          InductImpl[M, I_LISTENER](transport.id, stationId, arrivalStore, inductPhysics, acknowledgeInjector, restoreInjector)
        ).tap(i => _induct = i)
      ,
      i =>
        if i.stationId == stationId then AppSuccess(i)
        else AppFail.fail(s"${i.id} already bound to Station different that $stationId")
    )
  ...
  override def discharge(
      stationId: Id,
      lHost: Link.API.Physics,
      dHost: Discharge.API.Physics
    ...
    _discharge.fold(
      ...
      d =>
        if d.stationId == stationId then AppSuccess(d)
        else AppFail.fail(s"${d.id} already bound to Station different that $stationId")
    )
