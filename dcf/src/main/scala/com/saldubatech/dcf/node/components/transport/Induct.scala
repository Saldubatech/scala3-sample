...
object Induct:
  ...
  object API:
    ...
    /** These methods have "best effort" semantics in that they will restore as many cards as available in the induct up to the
      * maximum defined by the method.
      */
  ...
  object Environment:
    ...
    trait Listener extends Identified:
      ...
      def loadAccepted(at: Tick, atStation: Id, atInduct: Id, load: Material): Unit
...
trait InductMixIn[M <: Material, LISTENER <: Induct.Environment.Listener] extends Induct[M, LISTENER] with SubjectMixIn[LISTENER]:
  ...
  override def delivery(binding: Sink.API.Upstream[M]): Deliverer =
    new Deliverer() {
      override def deliver(at: Tick, loadId: Id): UnitResult =
        for {
          arrival <- fromOption(arrivalStore.available(at, loadId).headOption)
          o       <- origin
          _       <- binding.acceptMaterialRequest(at, o.stationId, o.id, arrival.material)
          c       <- arrivalStore.consume(at, loadId)
        } yield doNotify(l => l.loadDelivered(at, o.stationId, stationId, id, binding.id, arrival.material))
    }
  ...
  override def loadArriving(at: Tick, card: Id, load: M): UnitResult =
    for {
      ...
      _ <- physics.inductCommand(at, card, load).map { _ =>
             pendingInductions.provision(at, Transfer(at, card, load))
           }
      rs <- l.acknowledge(at, load.id)
    } yield doNotify(l => l.loadArrival(at, o.stationId, stationId, id, load))
  ...
  override def inductionFail(at: Tick, card: Id, loadId: Id, cause: Option[AppError]): UnitResult =
    pendingInductions.consume(at, card).flatMap { arr =>
      cause match
  ...
  override def inductionFinalize(at: Tick, card: Id, loadId: Id): UnitResult =
    ...
    } yield doNotify(_.loadAccepted(at, o.stationId, id, arrival.material))
