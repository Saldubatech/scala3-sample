...
import com.saldubatech.dcf.node.components.Controller.API.Listener
import com.saldubatech.dcf.node.components.connectors.Distributor.Scanner

object SortingMachine:
  class ProcessorFactory[M <: Material]
  (
    physics: Processor.Physics[M],
  ...
  object API:
    ...
    type Listener = Controller.API.Listener
  ...
  // (
  //   processorFactory: SortingMachine.ProcessorFactory[M],
  ...
  //       r1: (Distributor[M], Processor[M, Listener], Collector[M, Listener], Map[String, Scanner[M]], Map[String, Induct[M, Listener]]) <-
  ...
  //           (iP, tr) => tr.buildInduct(sId, iP).map{ i => i.id -> i }
  //         }.collectAll.map{ _.toMap }
  ...
  //         controllerFactory.build[M, Controller.API.Listener, LISTENER](s"Pusher", sId, router, inducts, monitoredIntakes, processor, discharges.map{ (id, d) => d.id -> d}, jobCleanUp)
  //     } yield
  //       val (obDistributor, _processor, inboundCollector, monitoredIntakes, inducts) = r1
  //       new SortingMachine[M]() {
...
trait SortingMachine[M <: Material] extends SortingMachine.Identity:
